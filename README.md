...
The tool is oriented to Jira Cloud REST API v3 and uses a simple `config.ini` for credentials and parameters.
...
- Robust HTTP handling with retries/backoff for 429/5xx responses.
- Parallel fetching for worklogs to improve performance.
- TLS control:
  - `verify_ssl` toggle
  - Custom `ca_bundle`
  - Windows trust store support (`certifi-win32`, optional)
...
High-level flow:
1) Parse CLI args and read `config.ini`.
...
3) Build a JQL range and query issues via `/rest/api/3/search/jql` (POST, paginated).
...
- Packages:
  ...
  - openpyxl
  - urllib3
  - certifi-win32 (optional, Windows trust)

You can install these with:
```bash
pip install requests pandas tqdm python-dateutil openpyxl urllib3 certifi-win32
```
...
4) Prepare `config.ini` (see Configuration section below).

## Configuration (config.ini)

The extractor reads settings from `config.ini` in INI format under the `[jira]` section.
...
- Windows trust store:
  - The tool attempts to support Windows trust via `certifi-win32` (optional). Install with:
    ```
    pip install certifi-win32
    ```
  - If `verify_ssl = true` and no `ca_bundle` is provided, Windows trust may be used when available.
...
- Rate limiting:
  - 429 or 5xx responses trigger exponential backoff (honors `Retry-After` when present).
...
- SoW field missing:
  - The tool tries to detect `customfield_11921`. If absent, it logs a warning and leaves the SoW column blank.
  - If your Jira uses a different ID for SoW, adjust the `SOW_FIELD_ID` constant in the script.
...
- Q: Can I customize the SoW field ID?
  - A: Yes, edit `SOW_FIELD_ID` in the script if your Jira’s custom field ID differs.

- Q: What pandas/openpyxl versions are required?
  - A: Use current stable versions; the tool writes `.xlsx` via `openpyxl`.

---

Keep your `config.ini` safe and do not commit real credentials/tokens to version control.
